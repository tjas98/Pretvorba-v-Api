...
var mymap1 = L.map('mapa_homepage_div').setView([46.03, 14.9], 8);
...
mymap1.addLayer(layer1); //dodamo layer na mapo
...
for (i = 0; i < tabelaMarkerjev.length; i++) {
    trenutniMarker = tabelaMarkerjev[i];
    L.marker(trenutniMarker, {myCustomId: tabelaIDjev[i]}).on('click', izvediDejanje).addTo(mymap1);
}
